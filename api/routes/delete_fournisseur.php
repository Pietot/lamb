...
try {
    ...
    $stmt = $pdo->prepare('DELETE FROM fournisseur WHERE id_fournisseur = :id_fournisseur');
    $stmt->bindValue(':id_fournisseur', $id_fournisseur);
    $stmt->execute();
    http_response_code(200);
    echo json_encode(['success' => true, 'message' => 'Fournisseur supprimé avec succès'], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
} catch (Exception $e) {
