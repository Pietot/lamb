              ...
              <table class="data-table">
                ...
                <tbody>
                  ...
                  <tr v-if="filteredUsers.length === 0">
                    <td colspan="100%" class="empty-message">Aucun utilisateur trouvé</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div v-for="role in rolesWithStats" :key="role.id_role" class="role-card">
              ...
              <div class="role-stats">
                <span class="users-count"
                  >{{ role.usersCount }} utilisateur{{ role.usersCount > 1 ? "s" : "" }}</span
                >
              </div>
              <div class="role-permissions">
                ...
                <ul class="permissions-list">
                  <li
                    v-for="permission in getRolePermissions(role.id_role).slice(0, 3)"
                    :key="permission"
                  >
                    {{ permission }}
                  </li>
        ...
        <div class="modal-body">
          <form @submit.prevent="handleCreateUser" class="user-form">
            ...
            <div class="form-group">
              ...
              <select id="role" v-model="newUserForm.id_role" class="form-select" required>
    ...
    setup() {
      ...
      const fetchData = async () => {
        ...
        try {
          ...
          const usersResponse = await fetch(VITE_API_URL + "get_table?table=utilisateur", {
            method: "GET",
            headers: {
          ...
          // Récupérer les rôles
          const rolesResponse = await fetch(VITE_API_URL + "get_table?table=role", {
            method: "GET",
            headers: {
              "Content-Type": "application/x-www-form-urlencoded",
            },
      ...
      const handleCreateUser = async () => {
        ...
        try {
          ...
          const response = await fetch(VITE_API_URL + "new_user", {
            method: "POST",
            headers: {
              "Content-Type": "application/x-www-form-urlencoded",
            },
          ...
          if (err.message === "Champs invalides") {
            ...
            createError.value =
              err.message || "Impossible de créer l'utilisateur. Veuillez réessayer.";
          }
        } finally {
      ...
      const filteredUsers = computed(() => {
        ...
        const fullName = user => `${user.prenom.toLowerCase()} ${user.nom.toLowerCase()}`;
